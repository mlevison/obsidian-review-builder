...
/xrefactor When the test cases use: expect(filterTemplateLines(content, templateLines)).toBe(expected); it can be difficult to parse the output. When they're switched to call the function under test and then use the following approach: expect(final.split("\n")).toEqual(expected.split("\n")); the output of a failing test is more readable please make this change for all test cases.

## Assistant

I understand you want to refactor the test cases to use a more readable assertion format. Let me first find the test files that use `filterTemplateLines`.

**Tool Call: grep "filterTemplateLines"**
Status: Completed
...
Now I'll refactor all the test cases to use the more readable `.toEqual()` with split arrays. Let me create a todo list for this task.Now I'll refactor the test file to use the more readable assertion format:
...
Now let me update the todo list and run the tests to verify everything still passes.
...
(node:875) Warning: `--localstorage-file` was provided without a valid path
 ...
 PASS  tests/utils/template-filter.test.ts
  template-filter
    filterTemplateLines - #{#tlf1a-2}: Line-by-line comparison
      ...
      ✓ shouldn't filter empty lines that match template
    filterTemplateLines - #{#tlf1a-4}: Heading preservation
      ✓ should always preserve headings even if they match template (1 ms)
    ...
    filterTemplateLines - #{#tlf1a-5}: Template availability handling
      ...
      ✓ should handle null template lines (1 ms)
    filterTemplateLines - Complex scenarios
      ✓ should handle real daily note example (1 ms)
      ✓ should handle markdown formatting in content
      ✓ should preserve lines with partial template matches
...
Perfect! I've successfully refactored all test cases in `tests/utils/template-filter.test.ts` to use the more readable assertion format.

## Summary

All 21 test cases have been updated from:
```typescript
expect(filterTemplateLines(content, templateLines)).toBe(expected);
```
...
This change makes test failures much more readable because:
- Instead of showing a single large string diff, it shows an array comparison
- Each line is displayed separately, making it easier to spot which specific lines differ
- The diff output is cleaner and easier to parse visually

All 21 tests still pass after the refactoring.
